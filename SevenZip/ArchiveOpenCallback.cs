                                                ...
                                                ICryptoGetTextPassword, IDisposable
    {
        ...
        private void Init(string fileName)
        {
            if (!String.IsNullOrEmpty(fileName))
            {
                ...
                if (baseName.Length > 0 && baseName[baseName.Length - 1] == '.')
                {
                    if (fileName.EndsWith("001") || File.Exists(baseName + "001"))
                    {
                        _fileInfo = new FileInfo(fileName);
                        _volumeFileNames.Add(fileName);
                        int index = 1;
                        if (fileName.EndsWith("001")) index = 2;
                    ...
                    {
                        baseName = baseName + fileName[baseName.Length];
                        var volName = baseName + "01";
                        if (File.Exists(volName))
                        {
                            ...
                            while (true)
                            {
                                _volumeFileNames.Add(volName);
                                index++;
                                volName = baseName + (index > 9 ? index.ToString() : "0" + index);
                                if (!File.Exists(volName)) break;
                            }
                            _fileInfo = new FileInfo(fileName);
                            _volumeFileNames.Add(fileName);
                        }
