...
namespace SevenZip
{
                                                   ...
                                                   IDisposable
    {
        ...
        /// <param name="extractor">            The owner of the callback.</param>
        public ArchiveExtractCallback(IInArchive archive, string directory, int filesCount, bool directoryStructure,
            List<uint> actualIndexes, Dictionary<uint, string> altPaths, string password, SevenZipExtractor extractor)
            : base(password)
        {
            Init(archive, directory, filesCount, directoryStructure, actualIndexes, altPaths, extractor);
        }
		                     ...
		                     outStream, AskMode askExtractMode)
        {
            ...
            if (askExtractMode == AskMode.Extract)
            {
                ...
                if (!_fileIndex.HasValue)
                {
                    ...
                    if (_actualIndexes == null || _actualIndexesFastCheck.TryGetValue(index, out dummyBool))
                    {
                        ...
                        fileName = null;
                        if (_altPaths != null)
                            _altPaths.TryGetValue(index, out fileName);

                        if (fileName == null) 
                        {
                            //SAB: Sanitize before sending to Path, as archive can have invalid chars in it for Windows.
                            var sanitizedEntryName = SanitizeStringAsValidPath(entryName);
                            fileName = Path.Combine(_directory, _directoryStructure? sanitizedEntryName : Path.GetFileName(sanitizedEntryName));
                        }
