...
namespace SevenZip
{
...
#endif
    {
        ...
        private IInStream GetArchiveStream(bool dispose)
        {
            ...
            {
                string zipNextVolumeName = _fileName.Substring(0, _fileName.Length - 2) + "01";
                string altSevenZipNextVolumeName = _fileName.Substring(0, _fileName.Length - 3) + "001";
                bool sevenZipMultiVolume = _fileName.EndsWith(".001", StringComparison.OrdinalIgnoreCase);

                bool altSevenZipVolumeNaming = File.Exists(altSevenZipNextVolumeName);
                bool zipMultiVolume = File.Exists(zipNextVolumeName);
                if (!(sevenZipMultiVolume || altSevenZipVolumeNaming || zipMultiVolume)) 
                     //|| (_volumeFileNames.Count == 1) // mmmmmmmm
                {
                    _archiveStream = new InStreamWrapper(
                        new ArchiveEmulationStreamProxy(new FileStream( _fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite), _offset),
                        dispose);
                }
                else
                {
                    _archiveStream = new InMultiStreamWrapper(_fileName, altSevenZipVolumeNaming, zipMultiVolume, dispose);
            ...
            ArchiveOpenCallback openCallback)
        {
            if (!_opened)
            {
                OperationResult op = OperationResult.Ok;
                op = OpenArchiveInner(archiveStream, openCallback);  
                if (op!= OperationResult.Ok)
                {
                        ...
                        try
                        {
                            ...
                            for (uint i = 0; i < _filesCount; i++)
                            {
                                try
                                {
                                    ...
                                    fileInfo.LastWriteTime = NativeMethods.SafeDateCast(data);
                                    _archive.GetProperty(i, ItemPropId.CreationTime, ref data);
                                    fileInfo.CreationTime = NativeMethods.SafeDateCast(data);
                                    _archive.GetProperty(i, ItemPropId.LastAccessTime, ref data);
                                    fileInfo.LastAccessTime = NativeMethods.SafeDateCast(data);
        ...
        private ArchiveExtractCallback GetArchiveExtractCallback(string directory, int filesCount,
                                                                 List<uint> actualIndexes, Dictionary<uint, string> altPaths)
        {
            var aec = String.IsNullOrEmpty(Password)
                      ? new ArchiveExtractCallback(_archive, directory, filesCount, PreserveDirectoryStructure, actualIndexes, altPaths, this)
                      : new ArchiveExtractCallback(_archive, directory, filesCount, PreserveDirectoryStructure, actualIndexes, altPaths, Password, this);
        ...
        public bool Check()
        {
            ...
            try
            {
                ...
                using (var aec = GetArchiveExtractCallback("", (int)_filesCount, null, null))
                {
                    try
                    {
                        CheckedExecute(
            ...
            finally
            {
                ...
                var archiveStream = _archiveStream as InStreamWrapper;
...
#pragma warning restore CC0004 // Catch block cannot be empty
        ...
        /// <param name="indexes">      indexes of the files in the archive file table.</param>
        [CLSCompliant(false)]
        public void ExtractFiles(string directory, Dictionary<uint, string> altPaths, params int[] indexes)
        {
            DisposedCheck();
            ClearExceptions();
            ...
            try
            {
                ...
                using ((archiveStream = GetArchiveStream(origIndexes.Count != 1)) as IDisposable)
                {
                    ...
                    try
                    {
                        using (var aec = GetArchiveExtractCallback(directory, (int) _filesCount, origIndexes, altPaths))
                        {
                            try
                            {
                                CheckedExecute(
                ...
                foreach (ArchiveFileInfo archiveFileInfo in ArchiveFileData)
                {
                    ...
                    if (extractFileCallbackArgs.ExtractToStream != null || extractFileCallbackArgs.ExtractToFile != null)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                using (var file = new FileStream(extractFileCallbackArgs.ExtractToFile, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.ReadWrite, 8192))
                                {
        ...
        public void ExtractArchive(string directory)
        {
            ...
            try
            {
                ...
                using ((archiveStream = GetArchiveStream(true)) as IDisposable)
                {
                    ...
                    try
                    {
                        using (var aec = GetArchiveExtractCallback(directory, (int) _filesCount, null, null))
                        {
                            try
                            {
                                CheckedExecute(
