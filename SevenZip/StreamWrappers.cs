...
namespace SevenZip
{
    ...
    internal class MultiStreamWrapper : DisposeVariableWrapper, IDisposable
    {
        ...
        protected MultiStreamWrapper(bool dispose, bool zip) : base(dispose) { MultiVolumeZip = zip; }
    ...
    internal sealed class InMultiStreamWrapper : MultiStreamWrapper, ISequentialInStream, IInStream
    {
        ...
        byte getByteInBuffer(byte[] data, int bufferSize, long startPosition, long location)
        {
            if (location < startPosition || location >= (startPosition + bufferSize)) return 0;
            return data[location - startPosition] ;
        }

         void setByteInBuffer(byte[] data, int bufferSize, long startPosition, long location, byte value)
         {
             if (location < startPosition || location >= (startPosition + bufferSize)) return;
             data[location - startPosition] = value;
         }

         void setIntInBuffer(byte[] data, int bufferSize, long startPosition, long location, int value)
         {
             setByteInBuffer(data, bufferSize, startPosition, location, (byte) (value & 255));
             setByteInBuffer(data, bufferSize, startPosition, location + 1, (byte) ((value >> 8) & 255));
             setByteInBuffer(data, bufferSize, startPosition, location + 2, (byte) ((value >> 16) & 255));
             setByteInBuffer(data, bufferSize, startPosition, location + 3, (byte) ((value >> 24) & 255));
         }

         int getIntInBuffer(byte[] data, int bufferSize, long startPosition, long location)
         {
             unchecked
             {
             int value = getByteInBuffer(data, bufferSize, startPosition, location) |
                 (getByteInBuffer(data, bufferSize, startPosition, location + 1) << 8) |
                 (getByteInBuffer(data, bufferSize, startPosition, location + 2) << 16) |
                 (getByteInBuffer(data, bufferSize, startPosition, location + 3) << 24) ;
             return value;
             }
         }
        ...
        public int Read(byte[] data, uint size)
        {
            ...
            if (zipFormat)
            {
                if (startPosition <= (StreamLength - 22) && StreamLength<= Position)
                {

                    if (getIntInBuffer(data, readCount, startPosition, StreamLength - 22) == 0x06054B50)
                    {
                        long volumeOffset = 0;
                        foreach (int number in StreamOffsets.Keys)
                        {
                            if (StreamOffsets[number].Key > volumeOffset)
                                volumeOffset = StreamOffsets[number].Key;
                        }
                        unchecked
                        {
                            long offset = getIntInBuffer(data, readCount, startPosition, StreamLength - 6) + volumeOffset;
                            setIntInBuffer(data, readCount, startPosition, StreamLength - 6, (int)offset);
                        }
                        setIntInBuffer(data, readCount, startPosition, StreamLength - 18, 0);
                    }
                }
            }
    ...
    internal sealed class OutMultiStreamWrapper : MultiStreamWrapper, ISequentialOutStream, IOutStream
    {
        ...
        public OutMultiStreamWrapper(string archiveName, int volumeSize, bool altNames) :
            base(true, false)
        {
